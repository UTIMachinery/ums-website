<template><main class="model-page" v-if="entry"><section class="hero"><div class="wrap"><NuxtLink :to="`/spec-library/${routeSlug}/cnc-lathes`" class="back">← {{ manufacturer }} CNC Lathe Library</NuxtLink><div class="kicker">UMS MACHINERY SPECIFICATION LIBRARY</div><h1>{{ manufacturer }} {{ entry.model }} Specifications</h1><p>Historical {{ manufacturer }} {{ entry.model }} CNC turning specifications organized by year and control from UMS machinery records.</p></div></section><section class="wrap section"><div class="warning"><strong>Historical specification information — not a machine-for-sale listing.</strong> These are specifications recorded on individual machines and may vary by year, control, configuration and optional equipment.</div><h2>Historical {{ entry.model }} Specifications by Year</h2><p class="intro">Each block below uses the detailed specification rows recorded for that individual UMS machine. Advertising-summary text is used only when detailed specification rows are unavailable.</p><SpecYearConfigurations :model="`${manufacturer} ${entry.model}`" :configurations="configurations" /><SpecInventoryMatches :manufacturer="manufacturer" :model="entry.model" machine-type="lathe" /><section class="cta"><div class="cta-kicker">NEED A MACHINE?</div><h2>Looking for a {{ manufacturer }} {{ entry.model }}?</h2><p>Tell Used Machinery Source what you need, or browse the CNC lathes we currently have available.</p><div class="cta-actions"><NuxtLink to="/equipment#tell-us-what-you-need" class="primary">Tell Us What You Need</NuxtLink><NuxtLink to="/equipment" class="secondary">View Current Equipment</NuxtLink></div></section></section></main></template>
<script setup>
...
const configurations=computed(()=>(props.entry.records||[]).filter(r=>(r.specs&&r.specs.length)||(r.recordedSpecs&&r.recordedSpecs.trim())).map(r=>({year:r.year||'Year not recorded',title:`${r.year||'Year not recorded'} ${props.manufacturer} ${props.entry.model}${r.fallback?' — recorded summary':''}`,control:r.control||`${props.manufacturer} CNC configuration`,specs:r.specs||[],recordedSpecs:r.fallback?r.recordedSpecs:'',note:r.fallback?`Historical UMS record #${r.invid}. Detailed specification rows were not available for this record, so its recorded advertising summary is shown instead.`:`Historical UMS record #${r.invid}`})))
...
const structuredData=computed(()=>({
  ...
  '@graph':[
    {
      '@type':'TechArticle',
      headline:`${props.manufacturer} ${props.entry.model} Specifications by Year`,
      description:`Historical ${props.manufacturer} ${props.entry.model} CNC lathe specifications, years, controls and configurations recorded by Used Machinery Source.`,
      url:canonical.value,
    ...
    },
    {
      ...
      itemListElement:[
        ...
        {'@type':'ListItem',position:3,name:`${props.manufacturer} CNC Lathes`,item:`https://www.usedmachinerysource.com/spec-library/${props.routeSlug}/cnc-lathes`},
        {'@type':'ListItem',position:4,name:`${props.manufacturer} ${props.entry.model} Specifications`,item:canonical.value}
      ]
    }
  ]
}))
