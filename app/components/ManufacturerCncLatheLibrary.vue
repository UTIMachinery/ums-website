  ...
  <main class="library-page">
    ...
    <section class="wrap section">
      ...
      <div class="additional-grid">
        <NuxtLink v-for="entry in filteredEntries" :key="entry.slug" :to="`/spec-library/${routeSlug}/${entry.slug}`" class="additional-link"><strong>{{ entry.model }}</strong><small class="config-count">{{ entry.count }} historical {{ entry.count===1?'configuration':'configurations' }}</small></NuxtLink>
      </div>
...
.config-count{display:block;margin-top:4px}</style>