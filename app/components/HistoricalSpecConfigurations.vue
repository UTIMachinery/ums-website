...
function specRows(config){const rows=[];let last='';for(const s of config.specs||[]){const section=heading(s.groupnames),label=clean(s.description),value=clean(s.specvalues);if(section&&section!==last){rows.push({section});last=section}if(label&&value)rows.push({label,value})}return rows}
function equipmentRows(config){const rows=[];let last='';const seen=new Set();for(const s of config.specs||[]){const value=clean(s.equippedwith);if(!value||seen.has(value))continue;const section=heading(s.groupnameew);if(section&&section!=='EQUIPPED WITH'&&section!==last){rows.push({section});last=section}rows.push({value});seen.add(value)}return rows}
