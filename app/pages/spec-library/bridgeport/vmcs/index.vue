<template>
  <main class="page">
    <section class="hero"><div class="wrap"><NuxtLink to="/spec-library/vmcs" class="back">← VMC Spec Library</NuxtLink><div class="kicker">UMS MACHINERY SPECIFICATION LIBRARY</div><h1>Bridgeport Vertical Machining Centers</h1><p>The Bridgeport VMC section currently includes 2 model pages covering 6 historical Bridgeport VMC records, including Interact and Torque Cut families.</p></div></section>
    <section class="wrap section"><h2>Core Bridgeport VMC Model Pages</h2><p class="intro">Bridgeport model designations are kept separate where the historical database shows meaningful model or control-generation differences.</p><input v-model="q" class="search" type="search" placeholder="Search Bridgeport VMC model"><div class="grid"><NuxtLink v-for="m in filtered" :key="m[1]" :to="`/spec-library/bridgeport/vmcs/${m[1]}`" class="card"><strong>{{m[0]}}</strong><span>{{m[2]}} historical records · {{m[3]}}–{{m[4]}}</span></NuxtLink></div></section>
...
<script setup>
import raw from '~/assets/data/bridgeport-vmc-core.js'
const q=ref('')
...
useSeoMeta({title:'Bridgeport VMC Specifications | Interact Torque Cut | UMS Spec Library',description:'Research historical Bridgeport vertical machining center specifications including Interact 412 and Torque Cut 30 models.'})
useHead({link:[{rel:'canonical',href:'https://www.usedmachinerysource.com/spec-library/bridgeport/vmcs'}]})
</script>
<style scoped>
