<template>
  <main class="page">
    <section class="hero"><div class="wrap"><NuxtLink to="/spec-library/vmcs" class="back">← VMC Spec Library</NuxtLink><div class="kicker">UMS MACHINERY SPECIFICATION LIBRARY</div><h1>Monarch Vertical Machining Centers</h1><p>The Monarch VMC section currently includes 4 model pages covering 16 historical Monarch VMC records, including VMC-45, VMC-75 and VMC-150 families.</p></div></section>
    <section class="wrap section"><h2>Core Monarch VMC Model Pages</h2><p class="intro">Monarch model designations are kept separate where the historical database shows meaningful model or control-generation differences.</p><input v-model="q" class="search" type="search" placeholder="Search Monarch VMC model"><div class="grid"><NuxtLink v-for="m in filtered" :key="m[1]" :to="`/spec-library/monarch/vmcs/${m[1]}`" class="card"><strong>{{m[0]}}</strong><span>{{m[2]}} historical records · {{m[3]}}–{{m[4]}}</span></NuxtLink></div></section>
...
<script setup>
import raw from '~/assets/data/monarch-vmc-core.js'
const q=ref('')
...
useSeoMeta({title:'Monarch VMC Specifications | VMC-45 VMC-75 VMC-150 | UMS Spec Library',description:'Research historical Monarch vertical machining center specifications including VMC-45, VMC-75 and VMC-150 models.'})
useHead({link:[{rel:'canonical',href:'https://www.usedmachinerysource.com/spec-library/monarch/vmcs'}]})
</script>
<style scoped>
