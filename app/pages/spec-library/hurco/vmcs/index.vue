<template>
  <main class="page">
    <section class="hero"><div class="wrap"><NuxtLink to="/spec-library/vmcs" class="back">← VMC Spec Library</NuxtLink><div class="kicker">UMS MACHINERY SPECIFICATION LIBRARY</div><h1>Hurco Vertical Machining Centers</h1><p>The first Hurco VMC section includes 7 model pages covering 26 historical Hurco records, including BMC, VM and VMX families.</p></div></section>
    <section class="wrap section"><h2>Core Hurco VMC Model Pages</h2><p class="intro">Hurco model designations are kept separate where the historical database shows meaningful BMC, VM and VMX variants.</p><input v-model="q" class="search" type="search" placeholder="Search Hurco VMC model"><div class="grid"><NuxtLink v-for="m in filtered" :key="m[1]" :to="`/spec-library/hurco/vmcs/${m[1]}`" class="card"><strong>{{m[0]}}</strong><span>{{m[2]}} historical records · {{m[3]}}–{{m[4]}}</span></NuxtLink></div></section>
    <section class="wrap section note"><div class="label">HISTORICAL DATA METHOD</div><h2>Exact model variants remain separate</h2><p>UMS preserves Hurco model and control-generation differences, including Ultimax-era and WinMax-era configurations.</p></section>
  </main>
</template>
<script setup>
import raw from '~/assets/data/hurco-vmc-core.js'
const q=ref('')
...
useSeoMeta({title:'Hurco VMC Specifications | BMC VM VMX | UMS Spec Library',description:'Research historical Hurco vertical machining center specifications including BMC-40, BMC-4020, VM-10i, VMX-42 and related models.'})
useHead({link:[{rel:'canonical',href:'https://www.usedmachinerysource.com/spec-library/hurco/vmcs'}]})
</script>
<style scoped>
