<template>
  <main class="page">
    <section class="hero"><div class="wrap"><div class="kicker">UMS MACHINERY SPECIFICATION LIBRARY</div><h1>Vertical Machining Centers (VMC)</h1><p>Research historical VMC manufacturers, model families and specifications, or browse vertical machining centers currently for sale.</p><div class="actions"><a href="#library" class="primary">Browse VMC Spec Library</a><NuxtLink to="/equipment" class="secondary">View Current Equipment</NuxtLink></div></div></section>
    <section id="current-machines" class="wrap section current-section">
      <div class="section-heading-row">
        <div>
          <div class="label sale-label">FOR SALE NOW</div>
          <h2>Vertical Machining Centers Currently Available</h2>
        </div>
        <NuxtLink to="/equipment" class="text-link">View all current equipment →</NuxtLink>
      </div>

      <div v-if="currentVmcs.length" class="machine-grid">
        <article v-for="machine in currentVmcs" :key="machine.InvID" class="machine-card">
          <div class="machine-image-wrap">
            <img
              v-if="machineCardImages[machine.InvID]"
              :src="`/Images/${machineCardImages[machine.InvID]}`"
              :alt="`Used ${machine.Year || ''} ${machine.Manufacturer || ''} ${machine.Model || ''} vertical machining center for sale`.replace(/\s+/g,' ').trim()"
              loading="lazy"
            />
...
async function loadMachineCardImages(){
  for(const machine of currentVmcs.value){
    try{
...
['Spindle Taper','Common VMC tapers include 40- and 50-taper configurations.'],
['Spindle Speed','Maximum RPM affects tooling and material applications.'],
['Spindle Horsepower','Indicates main spindle power available for cutting.'],
['Automatic Tool Changer','ATC capacity determines how many tools can remain available.'],
['Spindle Nose to Table','Affects usable workpiece and tooling height.'],
['CNC Control','Control generation affects programming, service and operator familiarity.']
]
useSeoMeta({title:'Vertical Machining Center Specifications | VMC Spec Library | UMS',description:'Research historical vertical machining center specifications by manufacturer and model in the UMS Machinery Specification Library.'})
useHead({link:[{rel:'canonical',href:'https://www.usedmachinerysource.com/spec-library/vmcs'}]})
</script>
<style scoped>
.page{color:#17273a;background:#fff;padding-bottom:60px}.wrap{max-width:1260px;margin:0 auto;padding:0 28px}.hero{background:linear-gradient(105deg,#071b33,#0d2c52);color:#fff;padding:52px 0;border-bottom:4px solid #f47b20}.kicker,.label{font-size:.78rem;font-weight:900;letter-spacing:.12em;color:#f47b20;margin-bottom:10px}.hero h1{font-size:clamp(2.2rem,4.5vw,3.6rem);margin:0 0 14px}.hero p{max-width:880px;font-size:1.1rem;line-height:1.7}.actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:22px}.actions a{padding:11px 16px;border-radius:5px;text-decoration:none;font-weight:800}.primary{background:#f47b20;color:#fff}.secondary{border:1px solid #fff;color:#fff}.section{padding-top:46px}.section h2{font-size:2rem;color:#0b2545;margin:0 0 12px}.intro{max-width:980px;line-height:1.7;color:#526579}.grid,.specs{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:18px;margin-top:24px}.card,.spec{border:1px solid #d8e0e8;border-radius:10px;padding:20px;background:#fff}.featured{border-top:4px solid #f47b20}.card h3,.spec h3{margin-top:0;color:#0b2545}.card p,.spec p{color:#526579;line-height:1.55}.card a{color:#1c4587;font-weight:800;text-decoration:none}.card span{color:#6a7888;font-size:.88rem;font-weight:700}.section-heading-row{display:flex;justify-content:space-between;align-items:end;gap:20px;margin-bottom:22px}.sale-label{color:#22733a}.text-link{color:#1c4587;font-weight:800;text-decoration:none;white-space:nowrap}.text-link:hover{text-decoration:underline}.machine-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:20px}.machine-card{overflow:hidden;background:#fff;border:1px solid #d8e0e9;border-radius:7px;box-shadow:0 4px 18px rgba(11,37,69,.07)}.machine-image-wrap{aspect-ratio:4/3;background:#eef2f6;display:flex;align-items:center;justify-content:center;overflow:hidden}.machine-image-wrap img{width:100%;height:100%;object-fit:cover}.machine-image-placeholder{color:#617386;font-weight:700;font-size:14px}.machine-card-copy{padding:18px}.machine-card h3{margin:0 0 7px;color:#0b2545;font-size:20px;line-height:1.25}.machine-title-link{color:#0b2545;text-decoration:none}.machine-title-link:hover{text-decoration:underline}.machine-type{margin:0 0 9px;color:#43566b;font-size:14px}.stock-number{margin:0 0 9px;font-size:13px;font-weight:800;color:#17273a}.adv-spec{margin:0 0 16px;color:#43566b;font-size:14px;line-height:1.5}.view-machine-button{display:inline-flex;align-items:center;justify-content:center;background:#1c4587;color:#fff!important;padding:10px 15px;border-radius:5px;font-size:14px;font-weight:800;text-decoration:none}.view-machine-button:hover{background:#0b2545}.no-current-machines{display:flex;align-items:center;justify-content:space-between;gap:28px;padding:24px 26px;border:1px solid #cfd8e3;border-left:5px solid #f47b20;border-radius:6px;background:#f8fafc}.no-current-machines h3{margin:0 0 6px;color:#0b2545;font-size:21px}.no-current-machines p{max-width:820px;margin:0;line-height:1.55;color:#43566b}.orange-button{flex:0 0 auto;background:#f47b20;color:#fff;text-decoration:none;font-weight:800;padding:11px 17px;border-radius:5px}.additional{margin-top:10px}.search{width:100%;max-width:620px;padding:13px 15px;border:1px solid #aeb8c4;border-radius:6px;font-size:17px;margin:10px 0 22px}.compact-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.compact-card{display:flex;flex-direction:column;gap:4px;border:1px solid #d8e0e8;border-radius:8px;padding:13px;text-decoration:none;color:#17273a}.compact-card:hover{border-color:#f47b20;background:#fff9f4}.compact-card strong{color:#0b2545}.compact-card span{font-size:.78rem;color:#6a7888}.guide{margin-top:20px}.specs{grid-template-columns:repeat(4,minmax(0,1fr))}@media(max-width:1050px){.machine-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:900px){.grid,.specs,.compact-grid,.machine-grid{grid-template-columns:1fr}.wrap{padding:0 18px}.section-heading-row{align-items:flex-start;flex-direction:column}.no-current-machines{flex-direction:column;align-items:flex-start}}
</style>