<template>
  <main class="page">
    <section class="hero"><div class="wrap"><NuxtLink to="/spec-library/vmcs" class="back">← VMC Spec Library</NuxtLink><div class="kicker">UMS MACHINERY SPECIFICATION LIBRARY</div><h1>Matsuura Vertical Machining Centers</h1><p>The first Matsuura VMC section includes 4 model pages covering 18 historical Matsuura records, including MC and RA vertical machining center families.</p></div></section>
    <section class="wrap section"><h2>Core Matsuura VMC Model Pages</h2><p class="intro">Matsuura model variants are kept separate where the historical database shows meaningful pallet, spindle, or configuration differences.</p><input v-model="q" class="search" type="search" placeholder="Search Matsuura VMC model"><div class="grid"><NuxtLink v-for="m in filtered" :key="m[1]" :to="`/spec-library/matsuura/vmcs/${m[1]}`" class="card"><strong>{{m[0]}}</strong><span>{{m[2]}} historical records · {{m[3]}}–{{m[4]}}</span></NuxtLink></div></section>
    <section class="wrap section note"><div class="label">HISTORICAL DATA METHOD</div><h2>Exact model variants remain separate</h2><p>Matsuura model variants are kept separate where the historical database shows meaningful pallet, spindle, or configuration differences.</p></section>
  </main>
</template>
<script setup>
import raw from '~/assets/data/matsuura-vmc-core.js'
const q=ref('')
...
useSeoMeta({title:'Matsuura VMC Specifications | MC RA | UMS Spec Library',description:'Research historical Matsuura vertical machining center specifications including MC-510V, MC-1000VS, MC-600VDC and RA-II-DC.'})
useHead({link:[{rel:'canonical',href:'https://www.usedmachinerysource.com/spec-library/matsuura/vmcs'}]})
</script>
<style scoped>
