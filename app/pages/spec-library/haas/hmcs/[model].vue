...
const modelName=computed(()=>historicalModelBySlug({manufacturer:'Haas',slug:String(route.params.model||''),machineFilter:isHmc}))
const configurations=computed(()=>modelName.value?historicalConfigurations({manufacturer:'Haas',model:modelName.value,machineFilter:isHmc}):[])
...
useSeoMeta({title:()=>machine.value?`Haas ${machine.value.name} HMC Specifications | UMS Spec Library`:'Haas HMC Specifications | UMS',description:()=>machine.value?`Historical Haas ${machine.value.name} horizontal machining center specifications including pallet, travels, spindle, ATC and control data.`:'Historical Haas HMC specifications.'})
const canonical=computed(()=>`https://www.usedmachinerysource.com/spec-library/haas/hmcs/${route.params.model}`)
