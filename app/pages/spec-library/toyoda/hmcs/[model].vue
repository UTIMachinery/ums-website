...
const modelName=computed(()=>historicalModelBySlug({manufacturer:'Toyoda',slug:String(route.params.model||''),machineFilter:isHmc}))
const configurations=computed(()=>modelName.value?historicalConfigurations({manufacturer:'Toyoda',model:modelName.value,machineFilter:isHmc}):[])
...
useSeoMeta({title:()=>machine.value?`Toyoda ${machine.value.name} HMC Specifications | UMS Spec Library`:'Toyoda HMC Specifications | UMS',description:()=>machine.value?`Historical Toyoda ${machine.value.name} horizontal machining center specifications including pallet, travels, spindle, ATC and control data.`:'Historical Toyoda HMC specifications.'})
const canonical=computed(()=>`https://www.usedmachinerysource.com/spec-library/toyoda/hmcs/${route.params.model}`)
