<template>
  <main class="page">
    <section class="hero"><div class="wrap"><NuxtLink to="/spec-library/vmcs" class="back">← VMC Spec Library</NuxtLink><div class="kicker">UMS MACHINERY SPECIFICATION LIBRARY</div><h1>Kitamura Vertical Machining Centers</h1><p>The first Kitamura VMC section includes 5 model pages covering 16 historical Kitamura records, including Bridgecenter and Mycenter families.</p></div></section>
    <section class="wrap section"><h2>Core Kitamura VMC Model Pages</h2><p class="intro">Kitamura model designations are kept separate where the historical database shows meaningful Mycenter and Bridgecenter variants.</p><input v-model="q" class="search" type="search" placeholder="Search Kitamura VMC model"><div class="grid"><NuxtLink v-for="m in filtered" :key="m[1]" :to="`/spec-library/kitamura/vmcs/${m[1]}`" class="card"><strong>{{m[0]}}</strong><span>{{m[2]}} historical records · {{m[3]}}–{{m[4]}}</span></NuxtLink></div></section>
    <section class="wrap section note"><div class="label">HISTORICAL DATA METHOD</div><h2>Exact model variants remain separate</h2><p>UMS normalizes obvious capitalization variants such as Bridgecenter 8 / BRIDGE CENTER 8 while keeping true model variants separate.</p></section>
  </main>
</template>
<script setup>
...
useSeoMeta({title:'Kitamura VMC Specifications | Mycenter Bridgecenter | UMS Spec Library',description:'Research historical Kitamura vertical machining center specifications including Mycenter and Bridgecenter models.'})
useHead({link:[{rel:'canonical',href:'https://www.usedmachinerysource.com/spec-library/kitamura/vmcs'}]})
</script>
<style scoped>
