...
const modelName=computed(()=>historicalModelBySlug({manufacturer:'Toshiba',slug:String(route.params.model||''),machineFilter:isHmc}))
const configurations=computed(()=>modelName.value?historicalConfigurations({manufacturer:'Toshiba',model:modelName.value,machineFilter:isHmc}):[])
...
useSeoMeta({title:()=>machine.value?`Toshiba ${machine.value.name} HMC Specifications | UMS Spec Library`:'Toshiba HMC Specifications | UMS',description:()=>machine.value?`Historical Toshiba ${machine.value.name} horizontal machining center specifications including pallet, travels, spindle, ATC and control data.`:'Historical Toshiba HMC specifications.'})
const canonical=computed(()=>`https://www.usedmachinerysource.com/spec-library/toshiba/hmcs/${route.params.model}`)
