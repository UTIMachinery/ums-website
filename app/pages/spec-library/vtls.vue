<template>
  <main class="page">
    ...
    <section id="current-machines" class="wrap section current-section">
      ...
      <div v-else class="no-current-machines"><div><h3>Looking for a vertical boring mill or VTL?</h3><p>We do not have a matching machine listed in current inventory right now. Send us your requirements and UMS can help locate one.</p></div><NuxtLink to="/machine-needed" class="orange-button">Tell Us What You Need</NuxtLink></div>
    </section>
    <section class="wrap section"><div class="label">BROWSE BY MANUFACTURER</div><h2>VBM / VTL Manufacturers</h2><p class="intro">This library contains {{ totalRecords }} historical UMS VBM/VTL records across {{ library.length }} manufacturers. The list is generated directly from historical-machines.json using WebDesc / Web_Desc, with models and year/configuration differences preserved from the historical records.</p>
      <input v-model="q" class="search" type="search" placeholder="Search VBM / VTL manufacturer">
      <div class="grid"><NuxtLink v-for="m in filtered" :key="m.slug" :to="`/spec-library/${m.slug}/vtls`" class="card"><strong>{{m.name}}</strong><span>{{m.models}} models · {{m.records}} historical records</span></NuxtLink></div>
      <p v-if="!filtered.length" class="empty">No VBM / VTL manufacturers match your search.</p>
    </section>
    ...
    <HistoricalLibraryFooter machine-type="Vertical Boring Mills / VTLs" />
  </main>
</template>
<script setup>
...
const machineType=m=>clean(m.WebDesc||m.Web_Desc).toLowerCase()
const isVtl=m=>machineType(m).startsWith('vertical boring mills, vtl')
const historicalVtls=historicalMachines.filter(isVtl)
const manufacturerMap=new Map()
for(const m of historicalVtls){
  const name=clean(m.Manufacturer)
  if(!name)continue
  const key=name.toLowerCase()
  if(!manufacturerMap.has(key))manufacturerMap.set(key,{name,slug:slugify(name),records:0,models:new Set()})
  const row=manufacturerMap.get(key)
  row.records++
  const model=clean(m.Model)
  if(model)row.models.add(model.toLowerCase())
}
const library=[...manufacturerMap.values()].map(m=>({...m,models:m.models.size})).sort((a,b)=>a.name.localeCompare(b.name))
const totalRecords=historicalVtls.length
...
const currentMachines=computed(()=>(machines.value||[]).filter(m=>Number(m.Sold)===0&&Number(offMarketValue(m))===0&&Number(m.dont_advertise)===0&&isVtl(m)).sort((a,b)=>Number(b.Year||0)-Number(a.Year||0)))
