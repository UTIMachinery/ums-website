<template>
  <main class="page">
    <section class="hero"><div class="wrap"><NuxtLink to="/spec-library/vmcs" class="back">← VMC Spec Library</NuxtLink><div class="kicker">UMS MACHINERY SPECIFICATION LIBRARY</div><h1>Leadwell Vertical Machining Centers</h1><p>The Leadwell VMC section currently includes 3 model pages covering 9 historical Leadwell VMC records, including MCV-1000P and MCV-1300 families.</p></div></section>
    <section class="wrap section"><h2>Core Leadwell VMC Model Pages</h2><p class="intro">Leadwell model designations are kept separate where the historical database shows meaningful model or control-generation differences.</p><input v-model="q" class="search" type="search" placeholder="Search Leadwell VMC model"><div class="grid"><NuxtLink v-for="m in filtered" :key="m[1]" :to="`/spec-library/leadwell/vmcs/${m[1]}`" class="card"><strong>{{m[0]}}</strong><span>{{m[2]}} historical records · {{m[3]}}–{{m[4]}}</span></NuxtLink></div></section>
...
<script setup>
import raw from '~/assets/data/leadwell-vmc-core.js'
const q=ref('')
...
useSeoMeta({title:'Leadwell VMC Specifications | MCV-1000P MCV-1300 | UMS Spec Library',description:'Research historical Leadwell vertical machining center specifications including MCV-1000P, MCV-1300 and MCV-1300P models.'})
useHead({link:[{rel:'canonical',href:'https://www.usedmachinerysource.com/spec-library/leadwell/vmcs'}]})
</script>
<style scoped>
