...
const modelName=computed(()=>historicalModelBySlug({manufacturer:'Cincinnati',slug:String(route.params.model||''),machineFilter:isHmc}))
const configurations=computed(()=>modelName.value?historicalConfigurations({manufacturer:'Cincinnati',model:modelName.value,machineFilter:isHmc}):[])
...
useSeoMeta({title:()=>machine.value?`Cincinnati ${machine.value.name} HMC Specifications | UMS Spec Library`:'Cincinnati HMC Specifications | UMS',description:()=>machine.value?`Historical Cincinnati ${machine.value.name} horizontal machining center specifications including pallet, travels, spindle, ATC and control data.`:'Historical Cincinnati HMC specifications.'})
const canonical=computed(()=>`https://www.usedmachinerysource.com/spec-library/cincinnati/hmcs/${route.params.model}`)
