<template>
  <main v-if="manufacturer&&machine" class="page">
    <section class="hero"><div class="wrap"><NuxtLink :to="`/spec-library/${manufacturer.slug}/grinders`" class="back">← {{manufacturer.name}} Grinder Library</NuxtLink><div class="kicker">UMS MACHINERY SPECIFICATION LIBRARY</div><h1>{{manufacturer.name}} {{machine.name}} Specifications</h1><p>Historical grinder specifications based on {{machine.records}} UMS machine record{{machine.records===1?'':'s'}}.</p></div></section>
...
const recordId=r=>String(r?.[3]||'')
const historicalIds=(machine.value?.years||[]).map(recordId).filter(Boolean)
const {data:historicalSpecs}=await useAsyncData(`grinder-specs-${route.path}`,()=>historicalIds.length?$fetch('/api/grinder-specs',{query:{ids:historicalIds.join(',')}}):[])
const detailedSpecs=id=>(historicalSpecs.value||[]).filter(row=>String(row[0])===String(id)&&cleanValue(row[3])).map(row=>({label:row[1]?`${String(row[1]).replace(/:$/,'')} — ${row[2]}`:row[2],value:row[3]}))
const summarySpecs=r=>{
 const out=[]
 if(cleanValue(r?.[2]))out.push({label:'Machine Type',value:r[2]})
