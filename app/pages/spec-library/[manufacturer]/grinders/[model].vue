...
const yearConfigurations=computed(()=>(machine.value?.years||[]).map(r=>{
  ...
  const fallback=(r[4]||r[2])?[{label:'Historical description',value:r[4]||r[2]}]:[]
  return{
    year:r[0],
    title:`${r[0]} ${manufacturer.value.name} ${machine.value.name} historical configuration`,
    control:r[1]||'Control not recorded',
    specs:details.length?details:fallback,
    note:`Historical record #${r[3]}`
  }
}).filter(r=>r.specs.length))
useSeoMeta({title:()=>manufacturer.value&&machine.value?`${manufacturer.value.name} ${machine.value.name} VBM Grinder Specifications | UMS Spec Library`:'VBM Grinder Specifications | UMS',description:()=>manufacturer.value&&machine.value?`Historical ${manufacturer.value.name} ${machine.value.name} Grinder specifications by year including table, swing, height, control and tooling data.`:'Historical Grinder specifications.'})
