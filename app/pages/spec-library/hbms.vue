<template>
  <main class="page">
    ...
    <section id="current-machines" class="wrap section current-section">
      ...
      <div v-else class="no-current-machines"><div><h3>Looking for a horizontal boring mill?</h3><p>We do not have a matching HBM listed in current inventory right now. Send us your requirements and UMS can help locate one.</p></div><NuxtLink to="/machine-needed" class="orange-button">Tell Us What You Need</NuxtLink></div>
    </section>
    <section class="wrap section"><div class="label">BROWSE BY MANUFACTURER</div><h2>HBM Manufacturers</h2><p class="intro">This library is generated directly from historical UMS records identified as horizontal boring mills by WebDesc / Web_Desc. Jig mills that shared broader database groups are excluded.</p>
      <input v-model="q" class="search" type="search" placeholder="Search HBM manufacturer">
      <div class="grid"><NuxtLink v-for="m in filtered" :key="m.slug" :to="`/spec-library/${m.slug}/hbms`" class="card"><strong>{{m.name}}</strong><span>Browse historical HBM models and specifications</span></NuxtLink></div>
      <p v-if="!filtered.length" class="empty">No HBM manufacturers match your search.</p>
    </section>
    ...
    <HistoricalLibraryFooter machine-type="Horizontal Boring Mills" />
  </main>
</template>
<script setup>
...
const library=historicalManufacturers({machineFilter:isHbm}).map(name=>({name,slug:slugify(name)}))
...
const currentMachines=computed(()=>(machines.value||[]).filter(m=>Number(m.Sold)===0&&Number(offMarketValue(m))===0&&Number(m.dont_advertise)===0&&isHbm(m)).sort((a,b)=>Number(b.Year||0)-Number(a.Year||0)))
