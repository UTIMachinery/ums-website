<template>
  <main class="page">
    ...
    <section id="current-machines" class="wrap section current-section">
      ...
      <div v-else class="no-current-machines"><div><h3>Looking for a grinder?</h3><p>We do not have a matching machine listed in current inventory right now. Send us your requirements and UMS can help locate one.</p></div><NuxtLink to="/machine-needed" class="orange-button">Tell Us What You Need</NuxtLink></div>
    </section>
    <section class="wrap section"><div class="label">BROWSE BY MANUFACTURER</div><h2>Grinder Manufacturers</h2><p class="intro">This library contains {{ totalRecords }} historical UMS grinder, lapper and hone records across {{ library.length }} manufacturers. The list is generated directly from historical-machines.json using WebDesc / Web_Desc.</p>
      <input v-model="q" class="search" type="search" placeholder="Search grinder manufacturer">
      <div class="grid"><NuxtLink v-for="m in filtered" :key="m.slug" :to="`/spec-library/grinders/${m.slug}`" class="card"><strong>{{m.name}}</strong><span>{{m.models}} models · {{m.records}} historical records</span></NuxtLink></div>
      <p v-if="!filtered.length" class="empty">No grinder manufacturers match your search.</p>
    </section>
    ...
    <HistoricalLibraryFooter machine-type="Grinders, Lappers & Hones" />
  </main>
</template>
<script setup>
...
const isGrinder=m=>machineType(m).startsWith('grinder')||machineType(m)==='hones'
const historicalGrinders=historicalMachines.filter(isGrinder)
const manufacturerMap=new Map()
for(const m of historicalGrinders){
...
const totalRecords=historicalGrinders.length
