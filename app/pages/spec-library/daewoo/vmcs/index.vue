<template>
  <main class="page">
    <section class="hero"><div class="wrap"><NuxtLink to="/spec-library/vmcs" class="back">← VMC Spec Library</NuxtLink><div class="kicker">UMS MACHINERY SPECIFICATION LIBRARY</div><h1>Daewoo Vertical Machining Centers</h1><p>The Daewoo VMC section currently includes 3 model pages covering 11 historical Daewoo VMC records, including DMV vertical machining center families.</p></div></section>
    <section class="wrap section"><h2>Core Daewoo VMC Model Pages</h2><p class="intro">Daewoo model designations are kept separate where the historical database shows meaningful machine or control-generation differences.</p><input v-model="q" class="search" type="search" placeholder="Search Daewoo VMC model"><div class="grid"><NuxtLink v-for="m in filtered" :key="m[1]" :to="`/spec-library/daewoo/vmcs/${m[1]}`" class="card"><strong>{{m[0]}}</strong><span>{{m[2]}} historical records · {{m[3]}}–{{m[4]}}</span></NuxtLink></div></section>
...
<script setup>
import raw from '~/assets/data/daewoo-vmc-core.js'
const q=ref('')
...
useSeoMeta({title:'Daewoo VMC Specifications | DMV-400 DMV-500 | UMS Spec Library',description:'Research historical Daewoo vertical machining center specifications including DMV-400, DMV-500 and DMV-4020 models.'})
useHead({link:[{rel:'canonical',href:'https://www.usedmachinerysource.com/spec-library/daewoo/vmcs'}]})
</script>
<style scoped>
