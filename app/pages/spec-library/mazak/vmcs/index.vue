<template>
  <main class="page">
    <section class="hero"><div class="wrap"><NuxtLink to="/spec-library/vmcs" class="back">← VMC Spec Library</NuxtLink><div class="kicker">UMS MACHINERY SPECIFICATION LIBRARY</div><h1>Mazak Vertical Machining Centers</h1><p>The first Mazak VMC section includes 10 model pages drawn from 87 historical Mazak machine records, including VTC, AJV, VQC and V-Series models.</p></div></section>
    <section class="wrap section"><h2>Core Mazak VMC Model Pages</h2><p class="intro">Older Mazak records often contain small metric-to-inch conversion differences. UMS preserves those differences and shows the most-supported recorded value rather than forcing unlike historical records into one specification.</p><input v-model="q" class="search" type="search" placeholder="Search Mazak VMC model"><div class="grid"><NuxtLink v-for="m in filtered" :key="m[1]" :to="`/spec-library/mazak/vmcs/${m[1]}`" class="card"><strong>{{m[0]}}</strong><span>{{m[2]}} historical records · {{m[3]}}–{{m[4]}}</span></NuxtLink></div></section>
    <section class="wrap section note"><div class="label">HISTORICAL DATA METHOD</div><h2>Specification confidence remains visible</h2><p>Each specification page shows how many historical observations matched the displayed value and flags fields where multiple recorded values exist. Fields with insufficient or obviously unusable historical data are omitted rather than guessed.</p></section>
  </main>
</template>
<script setup>
...
useSeoMeta({title:'Mazak VMC Specifications | VTC VQC AJV | UMS Spec Library',description:'Research historical Mazak vertical machining center specifications including VTC, VQC, AJV and V-Series models.'})
useHead({link:[{rel:'canonical',href:'https://www.usedmachinerysource.com/spec-library/mazak/vmcs'}]})
</script>
<style scoped>
