<template>
  <main class="page">
    <section class="hero"><div class="wrap"><NuxtLink to="/spec-library/vmcs" class="back">← VMC Spec Library</NuxtLink><div class="kicker">UMS MACHINERY SPECIFICATION LIBRARY</div><h1>Hitachi-Seiki Vertical Machining Centers</h1><p>The Hitachi-Seiki VMC section currently includes 2 model pages covering 7 historical Hitachi-Seiki VMC records, including VM-40 and VK-45 families.</p></div></section>
    <section class="wrap section"><h2>Core Hitachi-Seiki VMC Model Pages</h2><p class="intro">Hitachi-Seiki model designations are kept separate where the historical database shows meaningful model or control-generation differences.</p><input v-model="q" class="search" type="search" placeholder="Search Hitachi-Seiki VMC model"><div class="grid"><NuxtLink v-for="m in filtered" :key="m[1]" :to="`/spec-library/hitachi-seiki/vmcs/${m[1]}`" class="card"><strong>{{m[0]}}</strong><span>{{m[2]}} historical records · {{m[3]}}–{{m[4]}}</span></NuxtLink></div></section>
    <section class="wrap section note"><div class="label">HISTORICAL DATA METHOD</div><h2>Exact model variants remain separate</h2><p>UMS normalizes only obvious formatting or spelling duplicates while preserving true model differences.</p></section>
  </main>
</template>
<script setup>
import raw from '~/assets/data/hitachi-seiki-vmc-core.js'
const q=ref('')
...
useSeoMeta({title:'Hitachi-Seiki VMC Specifications | VM-40 VK-45 | UMS Spec Library',description:'Research historical Hitachi-Seiki vertical machining center specifications including VM-40 and VK-45 models.'})
useHead({link:[{rel:'canonical',href:'https://www.usedmachinerysource.com/spec-library/hitachi-seiki/vmcs'}]})
</script>
<style scoped>
