<template>
  <main class="page">
    <section class="hero"><div class="wrap"><NuxtLink to="/spec-library/vmcs" class="back">← VMC Spec Library</NuxtLink><div class="kicker">UMS MACHINERY SPECIFICATION LIBRARY</div><h1>Tree Vertical Machining Centers</h1><p>The Tree VMC section currently includes 2 model pages covering 15 historical Tree VMC records, including VMC-1050 and VMC-1060 families.</p></div></section>
    <section class="wrap section"><h2>Core Tree VMC Model Pages</h2><p class="intro">Tree model designations are kept separate where the historical database shows meaningful machine or control-generation differences.</p><input v-model="q" class="search" type="search" placeholder="Search Tree VMC model"><div class="grid"><NuxtLink v-for="m in filtered" :key="m[1]" :to="`/spec-library/tree/vmcs/${m[1]}`" class="card"><strong>{{m[0]}}</strong><span>{{m[2]}} historical records · {{m[3]}}–{{m[4]}}</span></NuxtLink></div></section>
...
<script setup>
import raw from '~/assets/data/tree-vmc-core.js'
const q=ref('')
...
useSeoMeta({title:'Tree VMC Specifications | VMC-1050 VMC-1060 | UMS Spec Library',description:'Research historical Tree vertical machining center specifications including VMC-1050 and VMC-1060 models.'})
useHead({link:[{rel:'canonical',href:'https://www.usedmachinerysource.com/spec-library/tree/vmcs'}]})
</script>
<style scoped>
