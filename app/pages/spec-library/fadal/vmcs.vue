<template>
  <main class="page">
    <section class="hero"><div class="wrap"><NuxtLink to="/spec-library/vmcs" class="back">← VMC Spec Library</NuxtLink><div class="kicker">UMS MACHINERY SPECIFICATION LIBRARY</div><h1>Fadal Vertical Machining Centers</h1><p>The first Fadal VMC section includes 10 model pages covering 112 historical Fadal records, including the 3016, 4020, 5020, 6030 and 8030 families.</p></div></section>
    <section class="wrap section"><h2>Core Fadal VMC Model Pages</h2><p class="intro">Fadal model designations are kept separate where the historical database shows meaningful variants such as HT, A and other configurations.</p><input v-model="q" class="search" type="search" placeholder="Search Fadal VMC model"><div class="grid"><NuxtLink v-for="m in filtered" :key="m[1]" :to="`/spec-library/fadal/vmcs/${m[1]}`" class="card"><strong>{{m[0]}}</strong><span>{{m[2]}} historical records · {{m[3]}}–{{m[4]}}</span></NuxtLink></div></section>
    <section class="wrap section note"><div class="label">HISTORICAL DATA METHOD</div><h2>Exact model variants remain separate</h2><p>UMS does not merge VMC-4020, VMC-4020A and VMC-4020HT into a single specification set. Each page uses the records associated with that exact model designation or only obvious formatting equivalents.</p></section>
  </main>
</template>
<script setup>
import raw from '~/assets/data/fadal-vmc-core.js'
const q=ref('')
