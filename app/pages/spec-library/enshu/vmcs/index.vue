<template>
  <main class="page">
    <section class="hero"><div class="wrap"><NuxtLink to="/spec-library/vmcs" class="back">← VMC Spec Library</NuxtLink><div class="kicker">UMS MACHINERY SPECIFICATION LIBRARY</div><h1>Enshu Vertical Machining Centers</h1><p>The Enshu VMC section currently includes 4 model pages covering 12 historical Enshu VMC records, including 650V, 650VX, 400FAV and VMC-430 families.</p></div></section>
    <section class="wrap section"><h2>Core Enshu VMC Model Pages</h2><p class="intro">Enshu model designations are kept separate where the historical database shows meaningful model or control-generation differences.</p><input v-model="q" class="search" type="search" placeholder="Search Enshu VMC model"><div class="grid"><NuxtLink v-for="m in filtered" :key="m[1]" :to="`/spec-library/enshu/vmcs/${m[1]}`" class="card"><strong>{{m[0]}}</strong><span>{{m[2]}} historical records · {{m[3]}}–{{m[4]}}</span></NuxtLink></div></section>
...
<script setup>
import raw from '~/assets/data/enshu-vmc-core.js'
const q=ref('')
...
useSeoMeta({title:'Enshu VMC Specifications | 650V 650VX VMC-430 | UMS Spec Library',description:'Research historical Enshu vertical machining center specifications including 650V, 650VX, 400FAV and VMC-430 models.'})
useHead({link:[{rel:'canonical',href:'https://www.usedmachinerysource.com/spec-library/enshu/vmcs'}]})
</script>
<style scoped>
