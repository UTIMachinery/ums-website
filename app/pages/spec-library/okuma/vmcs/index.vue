<template>
  <main class="page">
    <section class="hero"><div class="wrap"><NuxtLink to="/spec-library/vmcs" class="back">← VMC Spec Library</NuxtLink><div class="kicker">UMS MACHINERY SPECIFICATION LIBRARY</div><h1>Okuma Vertical Machining Centers</h1><p>The first Okuma VMC section includes 12 model pages covering 49 historical Okuma records, including MC, MX, Cadet, ES and VR families.</p></div></section>
    <section class="wrap section"><h2>Core Okuma VMC Model Pages</h2><p class="intro">Okuma model designations are kept separate where the historical database shows meaningful variants, while obvious naming duplicates such as Cadet Mate/Cadet-Mate are normalized.</p><input v-model="q" class="search" type="search" placeholder="Search Okuma VMC model"><div class="grid"><NuxtLink v-for="m in filtered" :key="m[1]" :to="`/spec-library/okuma/vmcs/${m[1]}`" class="card"><strong>{{m[0]}}</strong><span>{{m[2]}} historical records · {{m[3]}}–{{m[4]}}</span></NuxtLink></div></section>
    <section class="wrap section note"><div class="label">HISTORICAL DATA METHOD</div><h2>Exact model variants remain separate</h2><p>UMS keeps MC, MX, Cadet, ES and VR families separate and only consolidates obvious spelling or punctuation variants.</p></section>
  </main>
</template>
<script setup>
...
useSeoMeta({title:'Okuma VMC Specifications | MC MX Cadet ES | UMS Spec Library',description:'Research historical Okuma vertical machining center specifications including MC, MX, Cadet, ES and VR models.'})
useHead({link:[{rel:'canonical',href:'https://www.usedmachinerysource.com/spec-library/okuma/vmcs'}]})
</script>
<style scoped>
