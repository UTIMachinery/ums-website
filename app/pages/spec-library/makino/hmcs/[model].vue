...
const yearConfigurations=computed(()=>(machine.value?.years||[]).map(r=>({year:r[0],title:`${r[0]} Makino ${machine.value.name} historical configuration`,control:r[1]||'Control not recorded',specs:(r[2]||[]).filter(s=>s?.[0]&&s?.[1]&&String(s[1]).trim()).map(s=>({label:s[0],value:s[1]})),note:r[3]||''})))
useSeoMeta({title:()=>machine.value?`Makino ${machine.value.name} HMC Specifications | UMS Spec Library`:'Makino HMC Specifications | UMS',description:()=>machine.value?`Historical Makino ${machine.value.name} HMC specifications including pallets, travels, spindle, ATC and control data.`:'Historical Makino HMC specifications.'})
</script>
<style scoped>
