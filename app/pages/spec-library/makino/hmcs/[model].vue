<template>
  <main v-if="machine" class="page">
    <section class="hero"><div class="wrap"><NuxtLink to="../" class="back">← Makino HMC Spec Library</NuxtLink><div class="kicker">UMS MACHINERY SPECIFICATION LIBRARY</div><h1>Makino {{machine.name}} HMC Specifications</h1><p>Historical horizontal machining center specifications based on {{machine.records}} UMS machine record{{machine.records===1?'':'s'}}.</p></div></section>
    <section class="wrap section"><div class="warning"><strong>Historical reference information — not a machine-for-sale listing.</strong> Pallet, spindle, control and tooling configurations can vary by year and option package.</div><h2>Historical {{machine.name}} Specifications by Year</h2><p class="intro">Each block below represents a recorded historical year/control configuration.</p><HistoricalSpecConfigurations :configurations="configurations" /><SpecInventoryMatches manufacturer="Makino" :model="machine.name" machine-type="hmc" /></section>
  </main>
  <main v-else class="missing"><h1>Makino HMC model not found</h1><NuxtLink to="../">Browse Makino HMC specifications</NuxtLink></main>
</template>
<script setup>
...
const modelName=computed(()=>historicalModelBySlug({manufacturer:'Makino',slug:String(route.params.model||''),machineFilter:isHmc}))
const configurations=computed(()=>modelName.value?historicalConfigurations({manufacturer:'Makino',model:modelName.value,machineFilter:isHmc}):[])
...
useSeoMeta({title:()=>machine.value?`Makino ${machine.value.name} HMC Specifications | UMS Spec Library`:'Makino HMC Specifications | UMS',description:()=>machine.value?`Historical Makino ${machine.value.name} horizontal machining center specifications including pallet, travels, spindle, ATC and control data.`:'Historical Makino HMC specifications.'})
const canonical=computed(()=>`https://www.usedmachinerysource.com/spec-library/makino/hmcs/${route.params.model}`)
