...
const modelName=computed(()=>historicalModelBySlug({manufacturer:'OKK',slug:String(route.params.model||''),machineFilter:isHmc}))
const configurations=computed(()=>modelName.value?historicalConfigurations({manufacturer:'OKK',model:modelName.value,machineFilter:isHmc}):[])
...
useSeoMeta({title:()=>machine.value?`OKK ${machine.value.name} HMC Specifications | UMS Spec Library`:'OKK HMC Specifications | UMS',description:()=>machine.value?`Historical OKK ${machine.value.name} horizontal machining center specifications including pallet, travels, spindle, ATC and control data.`:'Historical OKK HMC specifications.'})
const canonical=computed(()=>`https://www.usedmachinerysource.com/spec-library/okk/hmcs/${route.params.model}`)
