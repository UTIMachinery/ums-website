<template>
  <main class="page">
    <section class="hero"><div class="wrap"><NuxtLink to="/spec-library/vmcs" class="back">← VMC Spec Library</NuxtLink><div class="kicker">UMS MACHINERY SPECIFICATION LIBRARY</div><h1>OKK Vertical Machining Centers</h1><p>The first OKK VMC section includes 6 model pages covering 14 historical OKK records, including MCV, PCV, VM and KVC families.</p></div></section>
    <section class="wrap section"><h2>Core OKK VMC Model Pages</h2><p class="intro">OKK model designations are kept separate where the historical database shows meaningful MCV, PCV, VM and KVC variants.</p><input v-model="q" class="search" type="search" placeholder="Search OKK VMC model"><div class="grid"><NuxtLink v-for="m in filtered" :key="m[1]" :to="`/spec-library/okk/vmcs/${m[1]}`" class="card"><strong>{{m[0]}}</strong><span>{{m[2]}} historical records · {{m[3]}}–{{m[4]}}</span></NuxtLink></div></section>
    <section class="wrap section note"><div class="label">HISTORICAL DATA METHOD</div><h2>Exact model variants remain separate</h2><p>UMS normalizes only obvious naming variants such as KVC-600 20L and KVC-600-20L while preserving true model differences.</p></section>
  </main>
</template>
<script setup>
import raw from '~/assets/data/okk-vmc-core.js'
const q=ref('')
...
useSeoMeta({title:'OKK VMC Specifications | MCV PCV VM KVC | UMS Spec Library',description:'Research historical OKK vertical machining center specifications including MCV-860, MCV-650, PCV-60, VM-7 and related models.'})
useHead({link:[{rel:'canonical',href:'https://www.usedmachinerysource.com/spec-library/okk/vmcs'}]})
</script>
<style scoped>
