<template>
  <main class="page">
    <section class="hero"><div class="wrap"><NuxtLink to="/spec-library/vmcs" class="back">← VMC Spec Library</NuxtLink><div class="kicker">UMS MACHINERY SPECIFICATION LIBRARY</div><h1>Milltronics Vertical Machining Centers</h1><p>The Milltronics VMC section currently includes 5 model pages covering 11 historical Milltronics VMC records, including VM, Partner and BR families.</p></div></section>
    <section class="wrap section"><h2>Core Milltronics VMC Model Pages</h2><p class="intro">Milltronics model designations are kept separate where the historical database shows meaningful machine or control-generation differences.</p><input v-model="q" class="search" type="search" placeholder="Search Milltronics VMC model"><div class="grid"><NuxtLink v-for="m in filtered" :key="m[1]" :to="`/spec-library/milltronics/vmcs/${m[1]}`" class="card"><strong>{{m[0]}}</strong><span>{{m[2]}} historical records · {{m[3]}}–{{m[4]}}</span></NuxtLink></div></section>
    <section class="wrap section note"><div class="label">HISTORICAL DATA METHOD</div><h2>Exact model variants remain separate</h2><p>UMS normalizes only obvious formatting or spelling duplicates while preserving genuine model differences.</p></section>
  </main>
</template>
<script setup>
import raw from '~/assets/data/milltronics-vmc-core.js'
const q=ref('')
...
useSeoMeta({title:'Milltronics VMC Specifications | VM Partner BR | UMS Spec Library',description:'Research historical Milltronics vertical machining center specifications including VM-16, VM-17, Partner and BR models.'})
useHead({link:[{rel:'canonical',href:'https://www.usedmachinerysource.com/spec-library/milltronics/vmcs'}]})
</script>
<style scoped>
