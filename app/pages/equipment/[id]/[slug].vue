<template>
  ...
  <section v-if="!machine && historicalMachine" class="unavailable-machine">
    <button type="button" class="back-to-equipment" @click="backToEquipment">‹ Back to Equipment</button>
    <div class="unavailable-card">
      <div class="unavailable-kicker">PREVIOUS EQUIPMENT LISTING</div>
      <h1>{{ historicalMachine.Year }} {{ historicalMachine.Manufacturer }} {{ historicalMachine.Model }}</h1>
      <p class="unavailable-stock">{{ historicalMachine.WebDesc }} &nbsp; • &nbsp; Stock #{{ historicalMachine.InvID }}</p>
      <h2>This machine is no longer available.</h2>
      <p>You may have reached this page through an older or cached search-engine link. You can still find specifications and information for this model in our Machine Specification Library, browse similar machines currently available, or let us know if you're looking for one.</p>
      <p><strong>Have one to sell or trade?</strong> We'd be happy to work with you or your OEM distributor if it's a trade-in.</p>
      <div class="unavailable-actions">
        <NuxtLink :to="historicalSpecLink" class="unavailable-primary">View {{ historicalMachine.Manufacturer }} {{ historicalMachine.Model }} Specifications</NuxtLink>
        <NuxtLink :to="historicalTypeLink" class="unavailable-secondary">View Available {{ historicalTypeLabel }}</NuxtLink>
        <NuxtLink to="/machine-needed" class="unavailable-secondary">Tell Us What You Need</NuxtLink>
        <NuxtLink :to="historicalSellLink" class="unavailable-secondary">Sell or Trade Your Machine</NuxtLink>
      </div>
    </div>
  </section>
...
import historicalMachinesData from '~/assets/data/historical-machines.json'
...
const historicalMachine=computed(()=>historicalMachinesData.find(item=>String(item.InvID)===String(route.params.id)))
function simpleSlug(value){return String(value||'').toLowerCase().trim().replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'')}
const historicalSpecLink=computed(()=>historicalMachine.value?`/spec-library/${simpleSlug(historicalMachine.value.Manufacturer)}/${simpleSlug(historicalMachine.value.Model)}`:'/spec-library')
const historicalTypeMap={
  'CNC Lathes & Turning Centers':['/equipment/type/cnc-lathes-turning-centers','CNC Lathes & Turning Centers'],
  'CNC Vertical Machining Centers and CNC Mills':['/equipment/type/vertical-machining-centers','Vertical Machining Centers'],
  'CNC Horizontal Machining Centers':['/equipment/type/horizontal-machining-centers','Horizontal Machining Centers'],
  'Vertical Boring Mills & VTL':['/equipment/type/vertical-boring-mills-vtls','VTLs & Vertical Boring Mills'],
  'Grinders, Lappers & Hones':['/equipment/type/grinders-lappers-hones','Grinders, Lappers & Hones'],
  'Fabricating':['/equipment/type/fabrication-machinery','Fabrication Machinery'],
  'EDM':['/equipment/type/edm-machines','EDM Machines'],
  'Engine Lathes, Drilling & Milling':['/equipment/type/engine-lathes-milling-drilling','Engine Lathes, Milling & Drilling Machines']
}
const historicalTypeLink=computed(()=>historicalTypeMap[String(historicalMachine.value?.Groups||'')]?.[0]||'/equipment')
const historicalTypeLabel=computed(()=>historicalTypeMap[String(historicalMachine.value?.Groups||'')]?.[1]||'Equipment')
const historicalSellLink=computed(()=>historicalMachine.value?`/sell-your-machine?manufacturer=${encodeURIComponent(historicalMachine.value.Manufacturer||'')}&model=${encodeURIComponent(historicalMachine.value.Model||'')}&year=${encodeURIComponent(historicalMachine.value.Year||'')}&type=${encodeURIComponent(historicalMachine.value.WebDesc||'')}`:'/sell-your-machine')
...
const seoRecord=computed(()=>machine.value||historicalMachine.value)
...
useSeoMeta({title:()=>machine.value?seoMachineTitle(machine.value):(historicalMachine.value?`${historicalMachine.value.Manufacturer} ${historicalMachine.value.Model} Specifications | UMS`:'Used Machinery Source'),description:()=>machine.value?buildMetaDescription(machine.value):(historicalMachine.value?`${historicalMachine.value.Manufacturer} ${historicalMachine.value.Model} Stock #${historicalMachine.value.InvID} is no longer available. View model specifications and currently available machinery from Used Machinery Source.`:'Used CNC machinery and industrial equipment from Used Machinery Source.'),ogTitle:()=>seoRecord.value?`${seoRecord.value.Manufacturer} ${seoRecord.value.Model}${seoRecord.value.Year?` – ${seoRecord.value.Year}`:''}`:'Used Machinery Source',ogDescription:()=>machine.value?`${machine.value.AdvSpec||''} Stock #${machine.value.InvID}.`:(historicalMachine.value?'This machine is no longer available. View model specifications and currently available machinery from Used Machinery Source.':'Used CNC machinery and industrial equipment from Used Machinery Source.'),ogImage:()=>machineImages.value?.length?`https://www.usedmachinerysource.com/Images/${machineImages.value[0]}`:'',ogUrl:()=>canonicalMachineUrl.value,ogType:'website',twitterCard:'summary_large_image',twitterTitle:()=>machine.value?`${machine.value.Manufacturer} ${machine.value.Model}${machine.value.Year?` – ${machine.value.Year}`:''}`:'Used Machinery Source',twitterDescription:()=>machine.value?`${machine.value.AdvSpec||''} Stock #${machine.value.InvID}.`:'Used CNC machinery and industrial equipment from Used Machinery Source.',twitterImage:()=>machineImages.value?.length?`https://www.usedmachinerysource.com/Images/${machineImages.value[0]}`:'',ogImageAlt:()=>machine.value?`Used ${machine.value.Year||''} ${machine.value.Manufacturer||''} ${machine.value.Model||''} ${seoMachineType(machine.value)} for sale`.replace(/\s+/g,' ').trim():'Used CNC machinery from Used Machinery Source',twitterImageAlt:()=>machine.value?`Used ${machine.value.Year||''} ${machine.value.Manufacturer||''} ${machine.value.Model||''} ${seoMachineType(machine.value)} for sale`.replace(/\s+/g,' ').trim():'Used CNC machinery from Used Machinery Source'})
useHead(()=>({
  htmlAttrs:{lang:'en'},
